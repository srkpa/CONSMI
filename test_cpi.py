...
def test(args):
    ...
    model.load_state_dict(torch.load("./ckpt/best_cpi_checkpoint.pt"))
    model.eval()
    ...
    with torch.no_grad():
        ...
        for test_smiles, test_protein, test_target in test_loader:
            smiles, protein, target = (
                test_smiles.to(args.device),
                test_protein.to(args.device),
                test_target.to(args.device),
            )
            ...
            test_acc.append(
                (test_output == target).float().mean().detach().cpu().numpy()
            )
